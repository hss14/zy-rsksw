...
if( !isset($_SESSION['valid_user']) ) {
	...
	try {
		...
		$query = "select * from recruit_test where id='".$id."'"; 
		$result = $db->query($query);
		if( (!$result) || ( $result->num_rows != 1 ) )
			throw new Exception("暂时无法在数据库中查询到您的报名信息。请您稍后再试。 ");
		$rct_assc = $result->fetch_assoc();

		$query = "select exam_name, level_code, level_position from exams where exam_id='02' and level_code='".$rct_assc['level_code']."'"; 
		$result = $db->query($query);
		if( (!$result) || ( $result->num_rows != 1 ) )
		...
		print_rct_table( $person_assc, $rct_assc, $exam_assc );
	} //end of try
	catch (Exception $e) {
...
function print_rct_table( $person_assc, $rct_assc, $exam_assc ) {
	...
	echo "\t\t\t</tr>\n"."\t\t\t<tr>\n";
		print_td('身份证号码');
		print_td( $rct_assc['id'], 3);
	echo "\t\t\t</tr>\n"."\t\t\t<tr>\n";
		print_td('工作单位');
		print_td( $rct_assc['company'], 3);
	echo "\t\t\t</tr>\n"."\t\t\t<tr>\n";
		print_td('申报学历');
		print_td( $rct_assc['shenbao_educ']);
		print_td('申报学历<br/>毕业时间');
		print_td( $rct_assc['shenbao_grad_date'] );
	echo "\t\t\t</tr>\n"."\t\t\t<tr>\n";
		print_td('申报学历专业');
		print_td( $rct_assc['shenbao_major'], 2);
		print_td('申报资格<br/>专业名称');
		print_td( $rct_assc['shenbao_for_major'] );
	echo "\t\t\t</tr>\n"."\t\t\t<tr>\n";
		print_td('现任专业<br/>技术职务');
		print_td( $rct_assc['position'], 2);
		print_td('任职时间');
		print_td( $rct_assc['company_date']);
	echo "\t\t\t</tr>\n"."\t\t\t<tr>\n";
		print_td('联系电话');
